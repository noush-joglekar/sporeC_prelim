...
from v1_chains import appendSingleDict, combineChunkedDicts
...
def process_chunk(dataDir, args,chunk):
    ...
    print("Range = ",start_file, end_file)

    for ix in range(start_file, end_file + 1):
        print(ix)
        filePath = f'{dataDir}/{args.inputDir}/incDict_{args.offDiagDist}_{args.prim_cutoff}_{args.sec_cutoff}_{ix}.pkl'
        if os.path.isfile(filePath):
            with open(filePath,'rb') as f:
                bIncDict = pickle.load(f)
                result_dict = appendSingleDict(bIncDict,result_dict)
...
def constructFullDict_pkl_parallel(dataDir, args):
    ...
    for chunk in range(1, num_chunks + 1):
        ...
        with open(temp_output_file, 'rb') as f:
            ...
            final_result_dict = combineChunkedDicts(result_dict,final_result_dict)
