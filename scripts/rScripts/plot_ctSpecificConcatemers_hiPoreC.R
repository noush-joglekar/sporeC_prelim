...
# Setup -----
# args <- c('/gpfs/commons/groups/gursoy_lab/ajoglekar/Projects/2023_03_01_multiwayInteractions/2023_03_01_v0_dataGathering/HiPoreC',
#           'differentialAnalysis/HiPoreC_cL/','K562','GM12878','3','chr11','differentialAnalysis/Plots_K562_vs_GM12878/',
...
ct1_bw_gr <- import.bw(ct1_bw)
ct1_tsa <- import.bw('tsaSeqData/K562_4DN.bw')
ct2_bw_gr <- import.bw(ct2_bw)
minR <- min(min(start(ct1_gr)),min(start(ct2_gr)))
maxR <- max(max(end(ct1_gr)),end(start(ct2_gr)))
view_range = GRanges(paste0(chrom,':',minR,'-',maxR))

pdf(paste0(plotDir,'both_',ct1,'_',ct2,'_card',card,'_',chrom,'_fullGR.pdf'),15,12,useDingbats = F)
plot(c(gTrack(ct1_bw_gr,lines = TRUE,y.field = "score",yaxis = F,height = 2, name='A/B'),
       gTrack(ct1_tsa,lines = TRUE,y.field = "score",yaxis = F,height = 2, name='TSA'),
      gTrack(split(ct1_gr, ct1_gr$readID) %>% unname, height = 10, name = ct1),
       gTrack(split(ct2_gr, ct2_gr$readID) %>% unname, height = 10, name = ct2),
      gTrack(ct2_bw_gr,lines = TRUE,y.field = "score",yaxis = F,height = 2, name='A/B')),view_range)
dev.off()
